...
class Astrometry(Tools):
    ...
    def query_vizier(self, catalog='usno', max_stars=100):
    ...
    def register(self, max_stars=300, max_stars_detect=60,
        ...
        ##     use_deep_frame=full_deep_frame)
        star_list_init_path = './temp/data.Astrometry.star_list' #####
        fwhm_arc = 1. #####
        ...
        ##     vmax=cutils.part_value(deep_frame.flatten(), 0.99.5),
        ##     cmap=pl.gray())
        ...
        ## ## optical distortions are too important. It could be used
        ## ## again with a good optical model.
        ## ## star_list_fit = np.copy(star_list_pix)

        self.target_x = 1027.103 ###
        self.target_y = 1030.978 ###
        deltax = 0.32309 / 3600. ###
        deltay = 0.32309 / 3600. ###
        self.wcs_rotation = 1.127 ###
        ...
        if compute_distortion:
            ...
            star_list_fit, index = get_filtered_params(
                fit_params, param='star_list', dist_min=10.,
                return_index=True)
            ...
            nonans = np.nonzero(~np.isnan(fit_params[:,'x']))
            w = 1./fit_params[:, 'x_err'][nonans]
            mapf = scipy.interpolate.SmoothBivariateSpline(
                fit_params[:, 'x'][nonans],
                fit_params[:, 'y'][nonans],
                fit_params[:,'dx'][nonans], s=None,
                w=w, kx=1, ky=1)
            dxmap = mapf(np.arange(self.data.shape[0]),
                         np.arange(self.data.shape[1]))
            dxmap[np.nonzero(dxmap > 15)] = np.nan
            dxmap[np.nonzero(dxmap < -15)] = np.nan
            ...
            #pl.scatter(fit_params[:, 'x'], fit_params[:, 'y'], c=fit_params[:,'dx'])
            pl.colorbar()
            pl.show()

            self._print_msg('Computing distortion polynomial (SIP)')
        ...
        if compute_precision:
            ...
            self._print_msg(
                "Astrometrical precision [in arcsec]: {:.3f} [+/-{:.3f}] computed over {} stars".format(
                    precision_mean, precision_mean_err, np.size(dx)))
    ...
    def brute_force_guess(self, image, star_list, x_range, y_range, r_range,
        ...
        box_size = int(3. * self.fwhm_pix)
        if not box_size&1: box_size += 1 # box_size must be odd
        kernel = cutils.gaussian_array2d(0., 1., box_size/2.-0.5,
                                         box_size/2.-0.5, self.fwhm_pix,
                                         box_size, box_size)
